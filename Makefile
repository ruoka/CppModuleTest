...
#c++m module wrapping headers etc.
	grep -HE '\s*export\s+module' $(sourcedir)/*.c++m | sed -r 's/^(.+)\/([a-z_0-9\-]+).c\+\+m:.+/$(objectdir)\/\2.o: $(objectdir)\/\2.pcm/' > $(dependencies)
#c++m module interface unit
	grep -HE '\s*import\s+([a-z_0-9]+)' $(sourcedir)/*.c++m | sed -r 's/^(.+)\/([a-z_0-9\-]+).c\+\+m:\s*import\s+([a-z_0-9]+);/$(objectdir)\/\2.pcm: $(objectdir)\/\3.pcm/' >> $(dependencies)
#c++m module partition unit
	grep -HE '\s*import\s+:([a-z_0-9]+)' $(sourcedir)/*.c++m | sed -r 's/^(.+)\/([a-z_0-9]+)(\-*)([a-z_0-9]*).c\+\+m:(.*)import\s+:([a-z_0-9]+);/$(objectdir)\/\2\3\4.pcm: $(objectdir)\/\2\-\6.pcm/' >> $(dependencies)
#c++m module implementation unit
	grep -HE '\s*module\s+([a-z_0-9]+)' $(sourcedir)/*.c++ | sed -r 's/^(.+)\/([a-z_0-9\-\.]+).c\+\+:\s*module\s+([a-z_0-9]+);/$(objectdir)\/\2.o: $(objectdir)\/\3.pcm/' >> $(dependencies)
#c++ source code
	grep -HE '\s*import\s+([a-z_0-9]+)' $(sourcedir)/*.c++ | sed -r 's/^(.+)\/([a-z_0-9\-\.]+).c\+\+:\s*import\s+([a-z_0-9]+);/$(objectdir)\/\2.o: $(objectdir)\/\3.pcm/' >> $(dependencies)
